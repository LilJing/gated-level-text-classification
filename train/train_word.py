...
import sys
import errno
from model.word.model_word_high import word_CNN
from helper.word_helper import AGNEWs
...
cnn.add_argument('-seq-len', type=int, default=112, help='the length of each batch')
cnn.add_argument('-highway-num', type=int, default=2, help='highway number')
...
experiment.add_argument('--save-folder', default='models_word_high1', help='Location to save epoch models, training configurations and results.')
...
def train(train_loader, dev_loader, model, args):
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        for i_batch, (data) in enumerate(train_loader, start=start_iter):
            ...
            # print inputs.shape 128*200*15
            # from IPython import embed;embed()
            # print inputs.shape
            # inputs = torch.FloatTensor(inputs)
            # targets = torch.FloatTensor(target)
            # print pd.DataFrame(inputs).shape
            # print target.shape
            # target.sub_(1)
            if args.cuda:
        ...
        if args.checkpoint and epoch % args.save_interval == 0:
            file_path = '%s/word_CNN_epoch_%d.pth.tar' % (args.save_folder, epoch)
        ...
        if best_acc is None or val_acc > best_acc:
            file_path = '%s/word_CNN_best.pth.tar' % (args.save_folder)
...
def main():
    ...
    train_dataset = AGNEWs(data_path=args.train_path)
    ...
    dev_dataset = AGNEWs(data_path=args.val_path)
